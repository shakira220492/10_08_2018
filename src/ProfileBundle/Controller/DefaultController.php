    ...
    public function getUserSpecificInformationAction(Request $request)
    {
        ...
        if ($request->isXMLHttpRequest()) {
            ...
            $views = $em->createQuery(
                "SELECT v.videoAmountViews 
                FROM HomeBundle:Video v 
                JOIN HomeBundle:User u 
                WITH v.user = u.userId 
                WHERE u.userId = '$userId'"
            );
            $views_v = $views->getResult();
            ...
            $amountViews = 0;
            while (isset($views_v[$amountViews]['videoAmountViews'])) {
                $amountViews += $views_v[$amountViews]['videoAmountViews'];
            }
